...
def lorentz(x, A, mu, gamma):
    return 1.0 - A * (gamma**2 / ((x - mu)**2 + gamma**2))

#-----------------LINEA DE BALMER-----------------

#Recortamos los datos alrededor de la linea de hidrogeno:
mascara_h = (w_norm >= 4310) & (w_norm <= 4370)
w_h = w_norm[mascara_h]
f_h = f_norm[mascara_h]

#Estimacion inicial a partir de la grafica:
p0_h = [0.55, 4340.5, 5.0]

#Ajuste con curve-fit:
popt_h, pcov_h = curve_fit(lorentz, w_h, f_h, p0=p0_h)
A_h_opt, mu_h_opt, gamma_h_opt = popt_h

#Generamos la curva suave:
flujo_ajustado_h = lorentz(w_h, *popt_h)

#Calculamos FWHM y EW:
fwhm_h = 2.0 * abs(gamma_h_opt)
...
plt.plot(w_h, flujo_ajustado_h, '-', color='purple', linewidth=2.5, alpha=0.8, label='Ajuste lorentziano')

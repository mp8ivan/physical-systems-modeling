...
#Analisis del centro de masas:
x_cm_num = [np.sum(x * np.abs(p)**2) * dx for p in psi] #centro de masas numerico en cada instante
k_potencial = 5.0**2 #frecuencia del potencial armónico
m_efectiva = 0.5 #masa efectiva
omega_fisica = np.sqrt(k_potencial / m_efectiva) #frecuencia corregida
x_teorico = x0 * np.cos(omega_fisica * t) #x(t) = x0 * cos(omega * t)

plt.figure(figsize=(10,5))
plt.plot(t, x_cm_num, 'b-', label='Centro de masa numerico')
plt.plot(t, x_teorico, 'r--', label='Prediccion clasica (m=0.5)')
plt.title('Comparacion del centro de masas')
plt.xlabel('t (s)')
plt.ylabel('x')
plt.legend()
